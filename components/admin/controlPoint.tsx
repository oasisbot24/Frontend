...
const ControlPoint = ({user, className}:Props) => {
  ...
  return (
      ...
      <div className="bg-white flex justify-between text-blue-200 rounded-t px-4 py-2">
        ...
        <button className="btn btn-blue-200 px-4 rounded-full h-[30px] my-auto"
      ...
      <form className="bg-gray-100 px-2 py-2" autoComplete='off' noValidate >
        <div className="flex justify-between">
          <div className="w-1/2 mr-2">
            <input className="h-[30px]" placeholder="±10000" type="text"
              {...register("amount", {
          ...
          <div className="w-1/2">
            <input className="h-[30px]" placeholder="사유" type="text"
              {...register("detail", {
        ...
        <div className="flex justify-between mt-2">
          <div className="text-xs font-semibold">
            {"분배후 포인트 : "}
            {user.point + (Number.isNaN(parseInt(watch('amount')?.toString())) ? 
            0 : parseInt(watch('amount').toString()))}
          </div>
