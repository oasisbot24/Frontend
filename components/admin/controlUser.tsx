  ...
  const submitHandler = async (data: PutAdminUserType) => {
    try {
      ...
      if (Number.isNaN(data.commission_rate) || Number.isNaN(data.nft))
        setError("root", {
          type: "fail",
          message: "수수료율과 NFT는 숫자만 입력해주세요",
        });
      else if (data.commission_rate < 0 || data.nft < 0)
        setError("root", {
          type: "fail",
          message: "수수료율과 NFT는 0보다 작을 수 없습니다",
        });
      else {
  ...
  return (
    <div className={className !== undefined ? className : ""}>
      <div className="bg-blue-200 flex justify-between text-white rounded-t px-4 py-2">
        ...
        <div className="flex">
          <button
            className="btn btn-red px-4 rounded-full h-[30px] my-auto mr-2"
          ...
          <button
            className="btn btn-white px-4 rounded-full h-[30px] my-auto"
            onClick={handleSubmit(submitHandler)}
          >
            <div className="flex">
              <Icon
                src="/static/icon/save.svg"
                className="w-[16px] mr-2 my-auto"
              />
      ...
      <form className="bg-gray-100 px-2 py-2 " autoComplete="off" noValidate>
        ...
        <div className="flex justify-between">
          <div className="commission_rate w-full mr-2">
            ...
            <input
              className={"h-[30px] " + (errors.commission_rate && "error")}
              placeholder="Rate"
              type="text"
          ...
          <div className="nft w-full ">
            ...
            <input
              className={"h-[30px] " + (errors.nft && "error")}
              placeholder="NFT"
              type="text"
              {...register("nft", {
