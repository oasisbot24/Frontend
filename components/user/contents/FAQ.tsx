  ...
  function activateContent(newContentID : string) {
    ...
    <div className="faq py-16">
      ...
      <div className="mt-12 lg:mx-12">
        ...
        {faq.map((group, index_group) => 
          <div key={index_group} className={"mt-3 "+group.className+(menu===group.className?'': ' hidden')}>
            {group.data.map((content, index_content) => 
              ...
              aria-expanded={contentID[content.id]}>
                <div className={"title flex justify-between px-4"}>
                  <div className="flex flex-col justify-center">
                    <div className="text-xs sm:text-sm lg:text-base font-normal text-white mb-0">{content.title}</div>
                  </div>
                  <Image className="p-2 w-fit" src={arrow} alt="arrow" width={100} height={100}/>
                </div>
                <div id={content.id} className="collapse-content">
                  <div className="content">
                    <div className="p-4 text-gray-200" dangerouslySetInnerHTML={{__html:content.content}}></div>
                  </div>
                </div>
              </div>
            )}
