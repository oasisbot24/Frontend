  ...
  return (
    <Card className={"h-full p-4 " + className}>
      <div className="flex justify-between mb-4">
        <div className="flex">
          <Icon
            src={"/static/coin/" + code.substr(4, 3).toLowerCase() + ".png"}
            className="mr-2"
          />
          <div className="font-semibold text-sm leading-[24px]">{code}</div>
        </div>
        {coin && coin.signed_change_rate >= 0 && (
          <Icon src="/static/icon/up.svg" className="w-[8px]" />
        )}
        {coin && coin.signed_change_rate < 0 && (
          <Icon src="/static/icon/down.svg" className="w-[8px]" />
        )}
      </div>
      <div className="flex justify-between">
        <div className="font-normal">
          <div className={"text-sm mb-1 highlight-text-" + animate}>
            {coin?.trade_price} KRW
          </div>
          <div
            className={
              "text-xs " +
              (coin && coin.signed_change_rate >= 0 ? "text-green" : "text-red")
            }
          >
            {coin && coin.signed_change_rate >= 0 && "+"}
